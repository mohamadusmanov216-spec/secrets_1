...
const startApplication = async (ctx: Context, chatId: number) => {
  ...
  try {
    const message = await ctx.telegram.sendMessage(chatId, 
    ...
    const userApp = applications.get(chatId.toString());
    if (userApp && message.message_id) {
      userApp.messageIds.push(message.message_id);
    }
  } catch (error) {
...
// Показать вопрос заявки
const showApplicationQuestion = async (ctx: Context, chatId: number, step: number) => {
  const questions = [
    `📝 *АНКЕТА ДЛЯ ТРЕНИРОВОК*\n\nВопрос 2/6:\n\nРост и вес?\n\n*Пример:* 180 см 75 кг`,
  ...
  if (step - 2 < questions.length) {
    try {
      const message = await ctx.telegram.sendMessage(chatId, questions[step - 2], {
        parse_mode: 'Markdown' as const,
        reply_markup: {
          inline_keyboard: [
            [{ text: '❌ Отменить заявку', callback_data: 'cancel_application' }]
  ...
  try {
    ...
    if (userApp && messageText !== '/start') {
      ...
      if (userApp.step <= 6) {
        ...
        if (userApp.step <= 6) {
          await showApplicationQuestion(ctx, parseInt(chatId), userApp.step);
        } else {
          // Заявка завершена
